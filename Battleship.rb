...
def show_board grid
  ...
  counter = 1
    letters.each do |letter|
      puts "  #{letter} | #{grid["#{letter}1"]} | #{grid["#{letter}2"]} | #{grid["#{letter}3"]} | #{grid["#{letter}4"]} | #{grid["#{letter}5"]} | #{grid["#{letter}6"]} | #{grid["#{letter}7"]} | #{grid["#{letter}8"]} | #{grid["#{letter}9"]} | #{grid["#{letter}10"]} "
      counter += 1
    end
end
...
def cpu_turn
  narrator("17")
  which_sunk(hit?(@player, @com_shot_board.keys.sample))
  puts "Computer Shot Board"
  show_board @player
  declare_winner
